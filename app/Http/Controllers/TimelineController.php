...
class TimelineController extends AppBaseController
{
    ...
    public function store(CreateTimelineRequest $request)
    {
        ...
        Flash::success('Línea de tiempo guardada correctamente.');
    ...
    public function showFeed(Request $request)
    {
        /*if(!Auth::check())
        {
            ...
            return view('maintenanceon');
        }*/
        
        // Si existe la variable de sesion redireccionar a la URL y remover de la sesion la variable _redirect
        if(session()->has('_redirect')) {
            return redirect()->away(session()->pull('_redirect'));
        }
        ...
        if (!$post->users_liked->contains(Auth::user()->id)) {
            ...
            if ($user_settings && $user_settings->email_like_post == 'yes') {
                Mail::send('emails.postlikemail', ['user' => $user, 'posted_user' => $posted_user], function ($m) use ($posted_user, $user) {
                    $m->from(Setting::get('noreply_email'), Setting::get('site_name'));
                    $m->to($posted_user->email, $posted_user->name)->subject($user->name.' '.'azotó tu publicación');
                });
            }
        ...
        if (!$comment->comments_liked->contains(Auth::user()->id)) {
            ...
            if ($user_settings && $user_settings->email_like_comment == 'yes') {
                Mail::send('emails.commentlikemail', ['user' => $user, 'comment_user' => $comment_user], function ($m) use ($user, $comment_user) {
                    $m->from(Setting::get('noreply_email'), Setting::get('site_name'));
                    $m->to($comment_user->email, $comment_user->name)->subject('a ' . $user->name .' le gusta tu comentario');
                });
            }
            ...
            return response()->json(['status' => '200', 'liked' => true, 'message' => 'ahora te gusta', 'likecount' => $like_count]);
        } else {
            ...
            return response()->json(['status' => '200', 'unliked' => false, 'message' => 'ahora no te gusta', 'likecount' => $like_count]);
        }
    }
    ...
    public function sharePost(Request $request)
    {
        ...
        if (!$post->users_shared->contains(Auth::user()->id)) {
            ...
            if ($post->user_id != Auth::user()->id) {
                ...
                if ($user_settings && $user_settings->email_post_share == 'yes') {
                    Mail::send('emails.postsharemail', ['user' => $user, 'posted_user' => $posted_user], function ($m) use ($user, $posted_user) {
                        $m->from(Setting::get('noreply_email'), Setting::get('site_name'));
                        $m->to($posted_user->email, $posted_user->name)->subject($user->name.' '.' compartió tu publicación');
                    });
                }
            }

            return response()->json(['status' => '200', 'shared' => true, 'message' => 'compartido correctamente', 'share_count' => $post_share_count]);
        } else {
            ...
            return response()->json(['status' => '200', 'unshared' => false, 'message' => 'Ya no está compartido', 'share_count' => $post_share_count]);
        }
    }
    ...
    public function pageLiked(Request $request)
    {
        ...
        if ($page->likes->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'like' => true, 'message' => 'ya no te gusta']);
        }
    }
    ...
    public function pageReport(Request $request)
    {
        ...
        if (!$timeline->reports->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'reported' => true, 'message' => 'reportado correctamente']);
        } else {
            ...
            return response()->json(['status' => '200', 'reported' => false, 'message' => 'reporte cancelado correctamente']);
        }
    }
    ...
    public function timelineGroups(Request $request)
    {
        ...
        if ($group->users->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'join' => true, 'message' => 'des unido correctamente']);
        }
    }
    ...
    public function show($id)
    {
        ...
        if (empty($timeline)) {
            Flash::error('Línea de tiempo no encontrada');
    ...
    public function update($id, UpdateTimelineRequest $request)
    {
        ...
        Flash::success('Línea de tiempo actualizada correctamente.');
    ...
    public function destroy($id)
    {
        ...
        Flash::success('Línea de tiempo eliminada correctamente.');
    ...
    public function follow(Request $request)
    {
        ...
        if (!$follow->followers->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'followed' => true, 'message' => 'seguido correctamente']);
        } else {
            ...
            return response()->json(['status' => '200', 'followed' => false, 'message' => 'ya no lo sigues']);
        }
    }
    ...
    public function joiningGroup(Request $request)
    {
        ...
        if (!$group->users->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'joined' => true, 'message' => 'unido correctamente']);
        } else {
            $group->users()->detach([Auth::user()->id]);
            ...
            return response()->json(['status' => '200', 'joined' => false, 'message' => 'des unido correctamente']);
        }
    }
    ...
    public function joiningClosedGroup(Request $request)
    {
        ...
        if (!$group->users->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'joinrequest' => true, 'message' => 'solicitud de unión al grupo correctamente enviada']);
        } else {
            ...
            if ($checkStatus && $checkStatus->status == 'approved') {
                ...
                return response()->json(['status' => '200', 'join' => true, 'message' => 'Solicitud sin éxito']);
            } else {
                $group->users()->detach([Auth::user()->id]);

                return response()->json(['status' => '200', 'joinrequest' => false, 'message' => 'Solicitud sin éxito']);
            }
        }
    }
    ...
    public function pageLike(Request $request)
    {
        ...
        if (!$page->likes->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'liked' => true, 'message' => 'Página azotada correctamente']);
        } else {
            $page->likes()->detach([Auth::user()->id]);
            ...
            return response()->json(['status' => '200', 'liked' => false, 'message' => 'Esta página ya no te gusta']);
        }
    }
    ...
    public function getNotifications(Request $request)
    {
        ...
        if (!$post->notifications_user->contains(Auth::user()->id)) {
            ...
            return response()->json(['status' => '200', 'notified' => true, 'message' => 'Correcto']);
        } else {
            $post->notifications_user()->detach([Auth::user()->id]);

            return response()->json(['status' => '200', 'unnotify' => false, 'message' => 'Incorrecto']);
        }
    }
    ...
    protected function groupPageValidator(array $data)
    {
        ...
        $messages = [
            'no_admin' => ':attribute está reservado para uso administrativo'
        ];
    ...
    public function pagesGroups($username)
    {
        ...
        if ($timeline->id == Auth::user()->timeline_id) {
            ...
            $theme = Theme::uses(Setting::get('current_theme', 'default'))->layout('default');
            $theme->setTitle('P&aacute;ginas y grupos | '.Setting::get('site_title').' '.Setting::get('title_seperator').' '.Setting::get('site_tagline'));
    ...
    public function deletePage(Request $request)
    {
        ...
        return response()->json(['status' => '200', 'deleted' => true, 'message' => 'Página eliminada correctamente']);
    }
    ...
    public function deletePost(Request $request)
    {
        ...
        return response()->json(['status' => '200', 'deleted' => true, 'message' => 'Publicación eliminada correctamente']);
    }
    ...
    public function reportPost(Request $request)
    {
        ...
        if ($reported) {
            ...
            return response()->json(['status' => '200', 'reported' => true, 'message' => 'Publicación reportada correctamente']);
        }
    }
