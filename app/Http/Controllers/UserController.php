...
class UserController extends AppBaseController
{
    ...
    {
        $messages['not_contains'] = 'El :attribute no debe contener palabras suspendidas';
    ...
    {
        $messages = [
            'no_admin' => 'El nombre "admin" est&aacute; reservado para uso administrativo',
            'username.required' => 'El nombre de usuario es requerido',
            'username.max' => 'El nombre de usuario debe ser menor a 16 caracteres',
            'username.min' => 'El nombre de usuario debe ser mayor a 5 caraceres',
            'name.required' => 'El nombre es obligatorio',
            'email.unique' => 'Ya existe una cuenta con ese correo electr&oacute;nico',
            'sex.required' => 'La orientaci&oacute;n sexual es obligatoria',
            'interest.required' => 'El inter&eacute;s principal es obligatorio',
    ...
    public function addingEventMembers(Request $request)
    {
        ...
        if ($request->user_status == 'Invited') {
            ...
            return response()->json(['status' => '200', 'added' => true, 'message' => 'satisfactoriamente des unido']);
        } else {
            $event->users()->attach($request->user_id, ['event_id' => $request->event_id]);

            return response()->json(['status' => '200', 'added' => true, 'message' => 'satisfactoriamente añadido']);
        }
    }
    ...
    public function movePhotos(Request $request)
    {
        if ($request->album_id == null) {
            Flash::error('Selecciona el álbum para mover las fotos');
            return redirect()->back();
        }
        if ($request->photos == null) {
            Flash::error('Selecciona al menos una foto');
            return redirect()->back();
        }
        ...
        Flash::success('Fotos seleccionadas movidas satisfactoriamente');
        return redirect()->back();
    }
    ...
    public function deletePhotos(Request $request)
    {
        ...
        Flash::success('Fotos Seleccionadas borradas satisfactoriamente');
        return redirect()->back();
    }
    ...
    public function relationshipSettings($username)
    {
        $users = User::all();
        $relationships = Relationship::where('user1_id', Auth::user()->id)
            ->orWhere('user2_id', Auth::user()->id)
            ->get();

        $theme = Theme::uses(Setting::get('current_theme', 'default'))->layout('default');
        $theme->setTitle(trans('common.relationships_settings').' '.Setting::get('title_seperator').' '.Setting::get('site_title').' '.Setting::get('title_seperator').' '.Setting::get('site_tagline'));

        return $theme->scope('users/settings/relationships', compact('users', 'relationships'))->render();
    }
}
