...
class LoginController extends Controller
{
    ...
    */
    ...
    public function getLogin()
    {
        ...
        if (Auth::user()) {
            if(isset($_GET['_redirect']))
                return redirect()->away($_GET['_redirect']);
            else
        ...
        return $theme->scope('users.login', compact('redirect'))->render();
    }
    ...
    public function login(Request $request)
    {
        ...
        if (!$validate->passes()) {
            return response()->json(['status' => '201', 'message' => trans('auth.login_failed'), 'redirect' => $request->_redirect]);
        } else {
            $user = '';
            $nameoremail = '';
            $canLogin = false;
        ...
        if($request->_redirect != NULL) {
            session()->flash('_redirect', $request->_redirect);
        }
