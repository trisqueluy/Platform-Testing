...
class AdminController extends Controller
{
    ...
    public function getMaintenaceMode()
    {
        $data = array();
        ...
        return $theme->scope('admin/maintenance-mode', compact('data'))->render();
    }
    
    public function switchMaintenanceMode(Request $request)
    {
        $data = array();
        $env = file_get_contents(base_path() . '/.env');
        $env = preg_split('/\r\n/', $env);
        ...
        if(count($env) > 0)
        {
            $write = file_put_contents(base_path() . '/.env', $env);
        }
        ...
        {
            $messages = 'Error al cambiar el modo de mantenimiento, es posible que tenga que restaurar el archivo de configuración';
            return redirect()
                ->back()
    ...
    public function deleteUser($userid)
    {
        ...
        $timeline = Timeline::where('username', $userid)->first();
        $user = $timeline->user;
        
        $pages = $user->pages;
        $groups = $user->groups;
        $posts = $user->posts;
        $pageLikes = $user->pageLikes;
        $roles = $user->roles;
        ...
        if(count($pageLikes) > 0) {
            foreach($pageLikes as $pageLike)
            {
                @$pageLike->delete();
            }
        }
        ...
        @$user->deleteOthers();
        @$user->deleteUserSettings($userid);
        //@$user->delete();
        //@$timeline->delete();
    ...
    public function passport()
    {
        ...
        return $theme->scope('admin/passport', compact('clients'))->render();
    }
}
